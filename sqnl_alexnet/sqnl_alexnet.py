...
class SqnlAlexNet(object):
    ...
    def create(self):
        ...
        conv1 = sqnlconv(self.X, 11, 11, 96, 1, 1, padding='SAME', name='conv1')
...
def sqnlconv(x, filter_height, filter_width, num_filters, stride_y, stride_x, name,
             padding='SAME'):
    """
    using square non-linearity instead of ReLU, didn't include bias
    """
    ...
    convolve = lambda i, k: tf.nn.conv2d(i, k,
                                         ...
                                         padding=padding)
    output_groups0 = convolve(x[:, :, :, 0:1], weights[:, :, 0:1, :]) ** 2
    output_groups1 = convolve(x[:, :, :, 1:2], weights[:, :, 1:2, :]) ** 2
    output_groups2 = convolve(x[:, :, :, 2:3], weights[:, :, 2:3, :]) ** 2

    output = tf.add(tf.add(output_groups0, output_groups1), output_groups2)
    return output
